...
import { ANALYTICS_EVENTS_QUEUE } from './rabbitmq.events';
...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      await this.channel.assertQueue(ANALYTICS_EVENTS_QUEUE, {
        durable: true,
      });
  ...
  async publishMessage(message: unknown): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error('Failed to publish analytics event', error as Error);
      return false;
    }
  }
