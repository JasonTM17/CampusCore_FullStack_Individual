import {
  Controller,
  Get,
  Post,
  Put,
  Delete,
  Body,
  Param,
  Query,
  UseGuards,
} from '@nestjs/common';
import { ApiTags, ApiOperation, ApiBearerAuth } from '@nestjs/swagger';
import { AttendanceService } from './attendance.service';
import { JwtAuthGuard } from '../auth/guards/jwt-auth.guard';
import { RolesGuard } from '../auth/guards/roles.guard';
import { Roles } from '../auth/decorators/roles.decorator';
import { CurrentStudent } from '../auth/decorators/current-user.decorator';
import { CurrentLecturer } from '../auth/decorators/current-user.decorator';

@ApiTags('Attendance')
@Controller('attendance')
@UseGuards(JwtAuthGuard, RolesGuard)
@ApiBearerAuth()
export class AttendanceController {
  constructor(private attendanceService: AttendanceService) {}

  // ============ ADMIN LECTURER ENDPOINTS ============

  @Post()
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Create attendance record' })
  create(@Body() data: any) {
    return this.attendanceService.create(data);
  }

  @Post('bulk')
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Create multiple attendance records' })
  createBulk(@Body() data: { records: any[] }) {
    return this.attendanceService.createBulk(data.records);
  }

  @Get()
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Get all attendance records' })
  findAll(
    @Query('page') page?: number,
    @Query('limit') limit?: number,
    @Query('sectionId') sectionId?: string,
    @Query('studentId') studentId?: string,
    @Query('date') date?: string,
  ) {
    return this.attendanceService.findAll(
      page || 1,
      limit || 20,
      sectionId,
      studentId,
      date,
    );
  }

  // ============ STUDENT ENDPOINTS ============

  @Get('my')
  @Roles('STUDENT')
  @ApiOperation({ summary: 'Get current student attendance' })
  getMyAttendance(
    @CurrentStudent() studentId: string,
    @Query('sectionId') sectionId?: string,
    @Query('semesterId') semesterId?: string,
  ) {
    return this.attendanceService.getStudentAttendance(
      studentId,
      sectionId,
      semesterId,
    );
  }

  @Get('my/summary')
  @Roles('STUDENT')
  @ApiOperation({ summary: 'Get current student attendance summary' })
  getMyAttendanceSummary(
    @CurrentStudent() studentId: string,
    @Query('semesterId') semesterId?: string,
  ) {
    return this.attendanceService.getStudentAttendanceSummary(
      studentId,
      semesterId,
    );
  }

  // ============ LECTURER ENDPOINTS ============

  @Get('lecturer/my')
  @Roles('LECTURER')
  @ApiOperation({ summary: 'Get attendance for lecturer sections' })
  getMySectionAttendance(
    @CurrentLecturer() lecturerId: string,
    @Query('sectionId') sectionId?: string,
    @Query('date') date?: string,
  ) {
    return this.attendanceService.getSectionAttendanceByLecturer(
      lecturerId,
      sectionId,
      date,
    );
  }

  @Get('section/:sectionId')
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Get attendance by section' })
  getSectionAttendance(
    @Param('sectionId') sectionId: string,
    @Query('date') date?: string,
  ) {
    return this.attendanceService.getSectionAttendance(sectionId, date);
  }

  @Get('section/:sectionId/summary')
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Get attendance summary by section' })
  getSectionAttendanceSummary(@Param('sectionId') sectionId: string) {
    return this.attendanceService.getSectionAttendanceSummary(sectionId);
  }

  @Post('section/:sectionId/mark')
  @Roles('LECTURER')
  @ApiOperation({ summary: 'Mark attendance for a section' })
  markSectionAttendance(
    @CurrentLecturer() lecturerId: string,
    @Param('sectionId') sectionId: string,
    @Body()
    data: {
      date: string;
      records: { studentId: string; status: string; notes?: string }[];
    },
  ) {
    return this.attendanceService.markSectionAttendance(
      lecturerId,
      sectionId,
      data.date,
      data.records,
    );
  }

  // ============ COMMON ENDPOINTS ============

  @Get(':id')
  @ApiOperation({ summary: 'Get attendance record by ID' })
  findOne(@Param('id') id: string) {
    return this.attendanceService.findOne(id);
  }

  @Put(':id')
  @Roles('ADMIN', 'SUPER_ADMIN', 'LECTURER')
  @ApiOperation({ summary: 'Update attendance record' })
  update(@Param('id') id: string, @Body() data: any) {
    return this.attendanceService.update(id, data);
  }

  @Delete(':id')
  @Roles('ADMIN', 'SUPER_ADMIN')
  @ApiOperation({ summary: 'Delete attendance record' })
  remove(@Param('id') id: string) {
    return this.attendanceService.remove(id);
  }
}
