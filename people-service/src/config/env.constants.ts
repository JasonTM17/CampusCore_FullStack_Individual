export const ENV = {
  NODE_ENV: 'NODE_ENV',
  PORT: 'PORT',
  DATABASE_URL: 'DATABASE_URL',
  FRONTEND_URL: 'FRONTEND_URL',
  SWAGGER_ENABLED: 'SWAGGER_ENABLED',
  COOKIE_SECURE: 'COOKIE_SECURE',
  HEALTH_READINESS_KEY: 'HEALTH_READINESS_KEY',
  JWT_SECRET: 'JWT_SECRET',
  RABBITMQ_URL: 'RABBITMQ_URL',
  AUTH_SERVICE_INTERNAL_URL: 'AUTH_SERVICE_INTERNAL_URL',
  ACADEMIC_SERVICE_INTERNAL_URL: 'ACADEMIC_SERVICE_INTERNAL_URL',
  INTERNAL_SERVICE_TOKEN: 'INTERNAL_SERVICE_TOKEN',
  SMTP_HOST: 'SMTP_HOST',
  SMTP_PORT: 'SMTP_PORT',
  SMTP_USER: 'SMTP_USER',
  SMTP_PASSWORD: 'SMTP_PASSWORD',
  SMTP_SECURE: 'SMTP_SECURE',
  EMAIL_FROM: 'EMAIL_FROM',
  EMAIL_FROM_NAME: 'EMAIL_FROM_NAME',
  PASSWORD_RESET_EXPIRY_MINUTES: 'PASSWORD_RESET_EXPIRY_MINUTES',
} as const;

export const ENV_DEFAULTS = {
  PORT: 4005,
  FRONTEND_URL: 'http://localhost',
  SWAGGER_ENABLED: false,
  COOKIE_SECURE: undefined,
  AUTH_SERVICE_INTERNAL_URL: 'http://auth-service:4007',
  ACADEMIC_SERVICE_INTERNAL_URL: 'http://academic-service:4003',
  INTERNAL_SERVICE_TOKEN: 'local-dev-internal-service-token-12345',
  EMAIL_FROM: 'noreply@campuscore.edu',
  EMAIL_FROM_NAME: 'CampusCore Administration',
  PASSWORD_RESET_EXPIRY_MINUTES: 15,
} as const;
