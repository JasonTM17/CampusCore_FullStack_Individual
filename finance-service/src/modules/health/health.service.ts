...
export class HealthService {
  ...
  liveness() {
    return {
      ...
      service: 'campuscore-finance-service',
    };
  }
  ...
  async metrics() {
    ...
    return [
      ...
      'campuscore_service_up{service="campuscore-finance-service"} 1',
      '# HELP campuscore_dependency_up Dependency status by service.',
      '# TYPE campuscore_dependency_up gauge',
      `campuscore_dependency_up{service="campuscore-finance-service",dependency="database"} ${services.database.status === 'up' ? 1 : 0}`,
      `campuscore_dependency_up{service="campuscore-finance-service",dependency="rabbitmq"} ${services.rabbitmq.status === 'up' ? 1 : 0}`,
      '# HELP campuscore_dependency_latency_ms Dependency latency in milliseconds.',
      '# TYPE campuscore_dependency_latency_ms gauge',
      `campuscore_dependency_latency_ms{service="campuscore-finance-service",dependency="database"} ${services.database.latency ?? 0}`,
      `campuscore_dependency_latency_ms{service="campuscore-finance-service",dependency="rabbitmq"} ${services.rabbitmq.latency ?? 0}`,
      '# HELP process_resident_memory_bytes Resident memory usage in bytes.',
      '# TYPE process_resident_memory_bytes gauge',
      `process_resident_memory_bytes{service="campuscore-finance-service"} ${rssBytes}`,
      '# HELP process_heap_used_bytes Heap usage in bytes.',
      '# TYPE process_heap_used_bytes gauge',
      `process_heap_used_bytes{service="campuscore-finance-service"} ${heapUsedBytes}`,
    ].join('\n');
  }
