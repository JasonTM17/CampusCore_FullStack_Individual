...
describe('Auth-service integration', () => {
  ...
  it('enforces CSRF for cookie refresh/logout and clears sessions on logout', async () => {
    const loginResponse = await request(baseUrl)
      .post('/api/v1/auth/login')
      .send(SEEDED_USERS.student)
      .expect(200);
    ...
    await request(baseUrl)
      .get('/api/v1/auth/me')
      .set('Cookie', toCookieHeader(logoutResponse.headers['set-cookie']))
      .expect(401);
  ...
  it('keeps public liveness minimal and readiness detailed', async () => {
  ...
  it('exposes canonical auth context and keeps people-context as a compatibility alias', async () => {
    const serviceToken =
      process.env.INTERNAL_SERVICE_TOKEN ?? 'integration-service-token';
    ...
    await request(baseUrl)
      .get(`/api/v1/internal/auth-context/users/${studentUser.id}`)
      .expect(403);
    ...
    await request(baseUrl)
      ...
      .expect(200)
      .expect(({ body }: SupertestResponse) => {
        expect(body.id).toBe(studentUser.id);
        expect(body.email).toBe(studentUser.email);
      });
  });

  it('restores student and lecturer shadow records from people-shadow events so jwt claims remain stable', async () => {
    const studentUser = await prisma.user.findUniqueOrThrow({
      where: { email: SEEDED_USERS.student.email },
