...
Repo hiện có bộ manifest Kustomize tại [k8s/README.md](./k8s/README.md) cho cùng topology 9 image, với `k8s/base` + `k8s/bootstrap` làm canonical deploy target, `k8s/overlays/docker-desktop` làm đường local-first cho Docker Desktop Kubernetes, `k8s/overlays/staging-generic` / `k8s/overlays/prod-generic` làm lớp cloud-agnostic công khai, và `k8s/overlays/staging-operator` / `k8s/overlays/prod-operator` làm lớp handoff cho operator dùng `ExternalSecret` + `Certificate`. Runtime này giữ nguyên boundary service, public routing, và security contract đang dùng ở Docker Compose. Để verify nhanh có cleanup mặc định, dùng `node scripts/run-k8s-local-smoke.mjs`; để giữ nguyên resources cho Docker Desktop UI, dùng `node scripts/run-k8s-local-deploy.mjs` rồi chuyển namespace sang `campuscore`; để mở edge local cho browser/IAB theo đường chính thức, dùng `node scripts/run-k8s-local-edge.mjs`. Helper này tự quản lý `kubectl port-forward`, tự hồi phục khi listener rơi bất ngờ, và coi log `wsarecv / error copying from local connection to remote stream` là client-disconnect noise nếu `/health` còn lên. Nếu chưa có IP public, `node scripts/run-cloudflare-tunnel-local.mjs` có thể expose local edge qua Cloudflare Tunnel; với Docker connector, Public Hostname service URL là `http://host.docker.internal:8080`. Lệnh `kubectl port-forward` thủ công chỉ còn là fallback debug. Khi cần dựng staging/prod thật, copy từ `k8s/templates/private-operator/staging` hoặc `k8s/templates/private-operator/prod` ra private overlay rồi điền hostname, ingress annotations, `ClusterSecretStore`, `ClusterIssuer`, và remote secret key thật. Chi tiết handoff ingress/TLS/secrets nằm tại [docs/K8S_HANDOFF.md](./docs/K8S_HANDOFF.md); đường đặt Cloudflare trước ingress nằm tại [docs/CLOUDFLARE.md](./docs/CLOUDFLARE.md).
