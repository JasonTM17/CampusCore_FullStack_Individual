...
`auth`, `users`, `roles`, `permissions` vẫn nằm ở `core-api`. Contract cookie, CSRF, internal headers, và helper JWT dùng chung được khóa qua package nội bộ `packages/platform-auth`.
...
Ở trạng thái hiện tại, release công khai phải có đủ **8 image**:
